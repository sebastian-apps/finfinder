...
def main():
    ...
    nbc_probs = jl.read_jsonfile("classifier-probs.json") # naive bayes classifier
    key_pages = {}
    filenum = 0
    errors_page = 0
    errors_file = 0
...
def bayesian(text, nbc_probs, category, likelihood_name):
    """
    INPUTS:
    ...
    likelihood_name (str): "in_class" or "not_in_class", whether or not it belongs to the category
    ...
    keyword_list = [keyword for keyword in nbc_probs.get(category).get(likelihood_name).keys() if keyword in text]
    # Calculate for ALL keywords, not just the ones found, as is above code.
    #keyword_list = [keyword for keyword in nbc_probs.get(category).get(likelihood_name).keys()]
    ...
    for keyword in keyword_list:
        p = likelihood*nbc_probs.get(category).get(likelihood_name).get(keyword)
...
def is_loner_present(num_list, threshold):
    ...
    Other possibilites: all values are close together, or all values are far apart (greater than threshold).
    """
